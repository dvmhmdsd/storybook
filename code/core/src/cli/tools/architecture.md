...
1. **Discover.** Read `~/.storybook/instances/*.json` (pid-liveness-checked). Match by cwd /
   configDir, then by `--port` when supplied. No record → local fallback, or a hard error under
   `--attach`. A project match with no instance on that port is a port-mismatch, not no-instance.
...
| Port mismatch                     | Project matches, none on `--port`     | The project's running ports; omit `--port` or retry with one of them                             |
| Multiple matches                  | 2+ records match                      | Matched instances with `configDir` and `port`; `--config-dir <dir>` or `--port <number>`         |
